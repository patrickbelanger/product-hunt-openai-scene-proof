...
class FfmpegAdapter(
    ...
    fun inspect(input: Path): Long {
        ...
        return (duration * 1000).roundToLong()
    }

    fun extract(input: Path, outputDirectory: Path = input.parent, maxSide: Int = 1600): PreparedMedia {
        require(maxSide in 1..1600)
        val duration = inspect(input) / 1000.0
        val inputOptions = listOf("-protocol_whitelist", "file", "-f", "mov", "-enable_drefs", "0", "-use_absolute_path", "0")
        val interval = String.format(Locale.ROOT, "%.6f", duration / 24)
        val spacing = String.format(Locale.ROOT, "%.6f", duration / 32)
        val filter = "setpts=PTS-STARTPTS,select='isnan(prev_selected_t)+gte(t-prev_selected_t,$spacing)*(gt(scene,0.3)+gte(t-prev_selected_t,$interval))',scale='min($maxSide,iw)':'min($maxSide,ih)':force_original_aspect_ratio=decrease,showinfo"
        val result = process.run(listOf(ffmpeg, "-nostdin", "-hide_banner", "-loglevel", "info", "-xerror", "-threads", "2") + inputOptions + listOf(
            ...
            "-filter_threads", "1", "-fps_mode", "vfr", "-frames:v", "32", "-threads", "2", "-start_number", "0", outputDirectory.resolve("%02d.png").toString(),
        ), 90)
        val times = Regex("\\bn:\\s*\\d+\\s+pts:.*?pts_time:([0-9.eE+-]+)").findAll(result)
            .map { (it.groupValues[1].toDouble() * 1000).roundToLong() }.toList()
        val files = Files.list(outputDirectory).use { entries -> entries.filter { it.fileName.toString().matches(Regex("[0-9]{2}\\.png")) }.sorted().toList() }
