...
import { decideFilmCandidate, getFilmCandidates, getFilmIntelligence, getFilmTranscript, promoteFilmCandidate, understandFilm, uploadSourceFilm, type DecideFilmCandidate, type FilmCandidate, type FilmEvidence, type FilmIntelligence as Intelligence, type FilmRun, type TranscriptSegment, type UnderstandFilmRequest } from '@sceneproof/api-client';

const stageLabels: Record<FilmRun['stage'], string> = {
...
const isActive = (run?: FilmRun) => !!run && run.stage !== 'SUCCEEDED' && run.stage !== 'FAILED';
...
function FilmEvidenceView({ evidence, intelligence, transcript }: { evidence: FilmEvidence; intelligence: Intelligence; transcript: TranscriptSegment[] }) {
  ...
  return <details className="film-evidence"><summary>Inspect supporting evidence ({evidence.frameIds.length} frames, {evidence.transcriptSegmentIds.length} transcript segments)</summary>
    <div className="film-frame-grid">{evidence.frameIds.map(id => {
      ...
      return frame ? <figure key={id}><img src={frame.url} alt={`Source film at ${timestamp(frame.sourceTimestampMs)}`} loading="lazy" onError={event => { event.currentTarget.alt = 'Source evidence image unavailable'; }} /><figcaption>{timestamp(frame.sourceTimestampMs)} · decoded frame</figcaption></figure> : <Text key={id} c="red.3">Frame metadata unavailable.</Text>;
    })}</div>
...
export function FilmIntelligence({ projectId }: { projectId: string }) {
  ...
  return <section aria-labelledby="film-intelligence-title" className="film-intelligence">
    <Group justify="space-between"><div><Title id="film-intelligence-title" order={2} size="h4">Film Intelligence</Title><Text size="sm" c="dimmed">Upload the film. Discover what matters. Confirm what should stay consistent.</Text></div><Button variant="subtle" size="compact-sm" loading={intelligence.isFetching} onClick={() => void intelligence.refetch()}>Refresh film state</Button></Group>
    ...
    {intelligence.data && <Stack mt="md" gap="md">
      ...
      {!!intelligence.data.runs.length && <Select label="Understanding history" value={run?.id ?? null} data={intelligence.data.runs.map(value => ({ value: value.id, label: `${new Date(value.startedAt).toLocaleString()} · ${stageLabels[value.stage]}` }))} onChange={value => { if (value) setParams(previous => { const next = new URLSearchParams(previous); next.set('filmRun', value); return next; }); }} />}
      {run && <div><Text role="status" fw={600}>{stageLabels[run.stage]}</Text><ol className="film-stages">{run.stages.map(value => <li key={value.stage}>{stageLabels[value.stage]}{value.stage === run.stage && isActive(run) ? ' · in progress' : value.stage === 'FAILED' ? '' : ' · recorded'} <time dateTime={value.startedAt}>{new Date(value.startedAt).toLocaleTimeString()}</time></li>)}</ol>{run.stage === 'FAILED' && <Alert role="alert" color="red">{run.failureMessage} This failed attempt remains in history. A new attempt needs fresh confirmation.</Alert>}</div>}
      {run?.result && <><div><Title order={3} size="h5">Understanding summary</Title><Text size="sm" mt="xs">{run.result.summary}</Text><Text size="xs" c="dimmed" mt="xs">{run.result.inspectedFrameIds.length} sampled frames · {run.result.inspectedSegmentIds.length} analysis segments · {run.result.entities.length} recurring entities · {run.result.narrativeCues.length} narrative cues · {run.result.potentialConcerns.length} potential concerns</Text>{run.result.warnings.map((warning, index) => <Text key={index} size="xs" c="yellow.3" mt="xs">{warning}</Text>)}</div>
        ...
        <details><summary>Potential continuity concerns ({run.result.potentialConcerns.length})</summary><Text size="xs" c="dimmed">Discovery questions, not confirmed errors. Difference alone is not a continuity error.</Text>{run.result.potentialConcerns.map((concern, index) => <Paper withBorder p="sm" mt="sm" key={index}><Text fw={600}>{concern.title}</Text><Text size="sm">{concern.explanation}</Text><Text size="xs" c="dimmed">{concern.uncertainty}</Text><FilmEvidenceView evidence={concern.evidence} intelligence={intelligence.data!} transcript={transcript.data ?? []} /></Paper>)}</details>
        <div><Title order={3} size="h5">Candidate continuity anchors</Title><Text size="xs" c="dimmed">Accept, edit or reject. Only confirmed anchors become continuity expectations. Reviewing candidates makes no AI call.</Text>{candidates.isPending && <Text role="status">Loading candidates…</Text>}{candidates.error && <Alert role="alert" color="red">{candidates.error.message}<Button variant="subtle" onClick={() => void candidates.refetch()}>Retry candidates</Button></Alert>}{candidates.data?.length === 0 && <Text size="sm">No supported anchors were proposed.</Text>}<Stack mt="sm">{candidates.data?.map(candidate => <CandidateReview key={candidate.id} candidate={candidate} projectId={projectId} intelligence={intelligence.data!} transcript={transcript.data ?? []} />)}</Stack></div>
      </>}
