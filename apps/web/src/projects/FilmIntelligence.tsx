...
export function FilmIntelligence({ projectId }: { projectId: string }) {
  ...
  return <section data-tour="film-intelligence" aria-labelledby="film-intelligence-title" className="film-intelligence">
    <Group justify="space-between"><div><Title id="film-intelligence-title" order={2} size="h4">Film Intelligence</Title></div><Button variant="subtle" size="compact-sm" loading={manualRefreshing} onClick={async () => { setManualRefreshing(true); try { await intelligence.refetch(); } finally { setManualRefreshing(false); } }}>Refresh now</Button></Group>
