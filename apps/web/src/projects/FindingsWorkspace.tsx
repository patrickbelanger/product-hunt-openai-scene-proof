  ...
  return <>
    ...
    <aside data-tour="findings" className="workspace-panel findings-panel" aria-labelledby="findings-title">
      <Stack gap="md">
        ...
        {analysis.data?.status === 'SUCCEEDED' && <Text role="status">Analysis complete.</Text>}
        ...
        {findings.isSuccess && findings.data.length === 0 && <div className="findings-empty"><Title order={2} size="h4">{analysis.data?.kind === 'TARGETED' ? 'Targeted re-evaluation' : analysis.data?.status === 'SUCCEEDED' ? 'No continuity issues were found.' : !analysisId && shots.isSuccess && shots.data.length === 0 ? 'No continuity analysis has been run yet.' : 'No saved findings here.'}</Title><Text size="sm" c="dimmed" mt="sm">{analysis.data?.kind === 'TARGETED' ? 'This run evaluates one existing finding. Open All saved findings and its creator history to inspect the judgement.' : analysis.data?.status === 'SUCCEEDED' ? 'This analysis completed successfully and reported no continuity issues in the reviewed evidence.' : 'If you have not run continuity analysis yet, import a shot and choose Run continuity analysis. An empty list alone cannot tell whether an earlier analysis completed with zero findings; open its saved analysis link to see its status. When findings appear, select one to compare evidence, copy a suggested correction, or explicitly resolve, dismiss or request re-evaluation. Film Intelligence concerns are proposals, not automatically saved findings. A difference alone is not a continuity error.'}</Text></div>}
