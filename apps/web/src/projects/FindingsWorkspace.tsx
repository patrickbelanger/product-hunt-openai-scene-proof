...
export function FindingsWorkspace({ projectId }: { projectId: string }) {
  ...
  return <>
    <MediaWorkspace projectId={projectId} finding={selected} findings={findings.data ?? []} frameId={frameId} onSelectFrame={setFrameId} />
    <aside className="workspace-panel findings-panel" aria-labelledby="findings-title">
      <Stack gap="md">
        ...
        {!!findings.data?.length && <><Text size="xs" c="dimmed">Page {page + 1} · {findings.data.length} saved findings on this page</Text><ul className="finding-list">{findings.data.map(finding => <li key={finding.id}><button type="button" className="finding-button" aria-pressed={selected?.id === finding.id} onClick={() => select(finding)}><span className={`severity-label severity-${finding.severity.toLowerCase()}`}>{findingLabel(finding.severity)} severity · {findingLabel(finding.category)}</span><strong>{finding.title}</strong><span>{finding.summary}</span></button></li>)}</ul></>}
