...
export function MediaWorkspace({ projectId, demo = false, active = true, finding, findings = [], frameId, onSelectFrame }: { projectId: string; demo?: boolean; active?: boolean; finding?: Finding; findings?: Finding[]; frameId?: string; onSelectFrame?: (id: string) => void }) {
  const cache = useQueryClient();
  const [file, setFile] = useState<File | null>(null);
  const [selection, setSelection] = useState<string>();
  const shots = useQuery({ queryKey: ['shots', projectId], queryFn: ({ signal }) => listShots(projectId, signal), retry: false });
  const film = useQuery({ queryKey: ['film', projectId], queryFn: ({ signal }) => getFilmIntelligence(projectId, signal), retry: false });
  ...
  const selectedSegment = film.data?.segments.find(segment => segment.id === selected?.shot.id);
